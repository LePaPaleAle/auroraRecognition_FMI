...
CRYSTAL_AURORA_PATH = '/home/users/moisio/testing/KEV/aurora'
CRYSTAL_NO_AURORA_PATH = '/home/users/moisio/testing/KEV/no-aurora'
CRYSTAL_STATS_PATH = '/home/users/moisio/testing/KEV/stats'

ASTRA_AURORA_PATH = 'moisio@astra.fmi.fi:/home/users/moisio/public_html/testing/KEV/aurora'
ASTRA_NO_AURORA_PATH = 'moisio@astra.fmi.fi:/home/users/moisio/public_html/testing/KEV/no-aurora'
ASTRA_STATS_PATH = 'moisio@astra.fmi.fi:/home/users/moisio/public_html/testing/KEV/stats/'
...
aurora_stats_file_name = CRYSTAL_STATS_PATH + '/aurora/aurora_list_KEV_' + current_month + '.txt'
aurora_stats_file_name_astra = aurora_stats_file_name.split('/')[-1]
no_aurora_stats_file_name = CRYSTAL_STATS_PATH + '/no-aurora/no-aurora_list_KEV_' + current_month + '.txt'
...
if not os.path.exists(aurora_stats_file_name):
    ...
    auroraFile.write('Images with aurora in Kevo during {}:\n'.format(current_month))
    auroraFile.write('Format: KEVO_YYYYMMDD_hh:mm:ss\n\n')
    auroraFile.close()

if not os.path.exists(no_aurora_stats_file_name):
    noAuroraFile = open(no_aurora_stats_file_name, 'w+')
    noAuroraFile.write('Images with no aurora in Kevo during {}:\n'.format(current_month))
    noAuroraFile.write('Format: KEVO_YYYYMMDD_hh:mm:ss\n\n')
    noAuroraFile.close()
...
current_aurora_stats_file_name = CRYSTAL_STATS_PATH + '/current_month_aurora_images_KEV.txt'
current_aurora_stats_file_name_astra = current_aurora_stats_file_name.split('/')[-1]
current_no_aurora_stats_file_name = CRYSTAL_STATS_PATH + '/current_month_no_aurora_images_KEV.txt'
...
auroraTarFileName = CRYSTAL_AURORA_PATH + '/aurora_KEV.tar.gz'
noAuroraTarFileName = CRYSTAL_NO_AURORA_PATH + '/no-aurora_KEV.tar.gz'
